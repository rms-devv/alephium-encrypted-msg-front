...
export const routes = {
  ...
  accountMessages: route("/account/messages"),

  messageThread: route(
    (messageId: string) => `/account/message/${messageId}`,
  ...
  fundingQrCode: route(
    (address: string) => `/funding/qr-code/${address}`,
    `/funding/qr-code/:address`,
  ),
  fundingProvider: route("/funding/provider"),
  token: route((tokenId: string) => `/tokens/${tokenId}`, "/tokens/:tokenId"),
  hideToken: route(
  ...
  ledgerEntry: route(
    (networkId: string, group: string | undefined, keyType: string) =>
      `/ledger/import/start/${networkId}/${group}/${keyType}`,
    "/ledger/import/start/:networkId/:group/:keyType",
  ),
  ledgerSelect: route(
    (group?: number) => `/ledger/import/select/${group}`,
    "/ledger/import/select/:group",
  ),
  ledgerDone: route(
    (group?: number) => `/ledger/import/done/${group}`,
    "/ledger/import/done/:group",
  ),
}
